...
async function runMigration() {
    try {
        console.log('Starting Target Setting menu migration...');
        // 1. Add target setting menu item
    ...
    `);
        console.log('✓ Target Setting menu item added');
    ...
    `);
        ...
        console.log('\nTarget Setting menu item assigned to:');
        result.rows.forEach((row) => {
            console.log(`  - ${row.role_name} role`);
        });
        process.exit(0);
    }
    catch (error) {
